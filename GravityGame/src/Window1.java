...
public  class Window1 extends JPanel implements MouseMotionListener,KeyListener,ActionListener,MouseListener
{
	...
	String[] objectChoices = { "Realistic","Light Grey", "Dark Grey","Black","White","Random"};
	...
	private void paintInterval() {
			...
			for(Object s1 : objects){
				if(s1.getMass()>total/5){
					...
					for(int i = 1 ; i<=numberOfObjects; i++){
						...
						int velocityVar = (int)(200/(s1.getDiameter())+17);
					
						objectsCopy.add(new Object(s1.getX() +(int)( Math.random()*locationVar-(locationVar/2)),s1.getY()+(int)( Math.random()*locationVar-(locationVar/2)),s1.getMass()/numberOfObjects,Math.random()*velocityVar-(velocityVar/2),Math.random()*velocityVar-(velocityVar/2),false,objectsColor,0.1,100));
						timeOut.add(20);
					}
					objects.remove(s1);
					return;
		...
		if(massSlider.getValue()>((varSlider.getValue()*numberSlider.getValue())/2)/3){
	...
	public void paint(Graphics g) {
		difficulty.setText("Difficulty: "+Integer.toString((int)difficultyInt));
		difficulty.repaint();
		super.paint(g);
		...
		if(start){
			...
			g2d.setFont(font);
			g2d.setColor(new Color(playerColor.getRed(),playerColor.getGreen(),playerColor.getBlue(),text));
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getActionCommand()=="begin"){
			...
			for(int i = 1 ; i<objectNumber; i++){
				...
				if(randomC){
					objects.add(new Object( (int)(Math.random()*1000),(int)(Math.random()*732),randomMass,Math.random()*2-1,Math.random()*2-1,false,new Color((int)(Math.random()*255),(int)( Math.random()*255),(int)(Math.random()*255)),0.2,0));  
				}
				if(realistic){
					objects.add(new Object( (int)(Math.random()*1000),(int)(Math.random()*732),randomMass,Math.random()*2-1,Math.random()*2-1,false,new Color((int)(Math.random()*80)+145,(int)( Math.random()*80)+80,(int)(Math.random()*80)+15),0.2,0));
				}
				else{

					objects.add(new Object( (int)(Math.random()*1000),(int)(Math.random()*732),randomMass,Math.random()*2-1,Math.random()*2-1,false,objectsColor,0.2,0));  
				}
